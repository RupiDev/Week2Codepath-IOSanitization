<?php
  ...
  function update_state($state) {
    ...
    $sql .= "WHERE id='" . mysqli_real_escape_string($db, $state['id']) . "' ";
    $sql .= "LIMIT 1;";
  ...
  function validate_territory($territory, $errors=array()) 
  {
    ...
    // custom validation
    elseif (!isTerritory($territory['name'])) 
    {
      $errors[] = "Name should only have the alphabet and a space";
   	}
    ...
    // custom validation 
    elseif (!isPostion($territory['position']))
    {
      $errors[] = "Position can only be digits";
    }
  ...
  function update_territory($territory) {
    ...
    $sql .= "WHERE id='" . mysqli_real_escape_string($db, $territory['id']) . "' ";
    $sql .= "LIMIT 1;";
  ...
  function validate_salesperson($salesperson, $errors=array()) 
  {
    ...
    // custom validation
    elseif (!isFirstNameValid($salesperson['first_name'])) 
    {
    ...
    // custom validation
    elseif (!isLastNameValid($salesperson['last_name'])) 
    {
      $errors[] = "Last name should only have the alphabet, -, ., and a space";
    }
    ...
    if (is_blank($salesperson['phone'])) {
      ...
      $errors[] = "salesperson must be less than 12 characters.";
    }
    return $errors;
  }
  ...
  function insert_salesperson($salesperson) {
	...
	// used mysqli to avoid mysql injections 
  ...
  function validate_user($user, $errors=array()) 
  {
    ...
    // custom validation
    elseif (!isFirstNameValid($user['first_name'])) 
    {
      $errors[] = "First name should only have the alphabet, -, ., and a space";
    }
    ...
    // custom validation
    elseif (!isLastNameValid($user['last_name']))
    {
      $errors[] = "Last name should only have the alphabet, -, ., and a space";
    }
    ...
    // custom validation
    elseif (checkDuplicateUsername($user['username'], $db))
    {
      $errors[] = "Can't have the same username";
    }
    return $errors;
  }
  ...
  function insert_user($user) {
    ...
    $sql .= "'" . mysqli_real_escape_string($db, $user['username']) . "',";
    $sql .= "'" . mysqli_real_escape_string($db, $created_at) . "' ";
    $sql .= ");";
    // For INSERT statments, $result is just true/false
    $result = db_query($db, $sql);
    if($result) {
  ...
  function update_user($user) {
    ...
    $sql .= "first_name='" . mysqli_real_escape_string($db, $user['first_name']) . "', ";
    $sql .= "last_name='" . mysqli_real_escape_string($db, $user['last_name']) . "', ";
    $sql .= "email='" . mysqli_real_escape_string($db, $user['email']) . "', ";
    $sql .= "username='" . mysqli_real_escape_string($db, $user['username']) . "' ";
    $sql .= "WHERE id='" . mysqli_real_escape_string($db, $user['id']) . "' ";
    $sql .= "LIMIT 1;";
